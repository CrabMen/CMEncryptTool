...
#pragma mark - 加密 & 解密数据
- (NSData *)cm_encryptData:(NSData *)plainData {
...
- (NSData *)cm_decryptData:(NSData *)cipherData {
    OSStatus sanityCheck = noErr;
    size_t cipherBufferSize = 0;
    size_t keyBufferSize = 0;
    ...
    privateKey = [self cm_getPrivateKeyRef];
 ...
 *  生成密钥对
 */
- (void)cm_generateKeyPair:(NSUInteger)keySize {
    OSStatus sanityCheck = noErr;
    publicKeyRef = NULL;
    privateKeyRef = NULL;
 ...
 *  加载公钥
 */
- (void)cm_loadPublicKey:(NSString *)publicKeyPath {
 ...
 *  加载私钥
 */
- (void)cm_loadPrivateKey:(NSString *)privateKeyPath password:(NSString *)password {
